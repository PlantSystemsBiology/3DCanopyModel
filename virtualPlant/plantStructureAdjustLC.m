...
%         size(LeafLenPath)
        if length(LeafLenPath)<10 % 当叶片很短的时候，不调整弯曲度
        ...
        pts_oneLeaf2 = pts_oneLeaf; % pts_oneLeaf2 是每次被截短后的部分
        [row,col] = size(pts_oneLeaf2);

        indexLeafSegs = zeros(row,0); %用于保存每段叶片的index 0 or 1.
        preRotationDegree_Save = zeros(1,0); %
        turnAngleDirection_Save = zeros(1,0); %
        O_Pt_base_Save = zeros(0,3); %

        serials = [1:length(pts_oneLeaf2)]' ; % 原始点的序列号，1列
        pts_oneLeaf2 = [pts_oneLeaf2, serials]; % 将序列号放在点的第四列上
        pts_oneLeaf_straight = pts_oneLeaf2; % 用于保存调整后的点，初始化与之前的相同
        ...
        while(true)
           % k
            ...
            if ( 1+(k+1)*N >= length(LeafLenPath)) %判断是否到头了
                %             disp('x1')
            ...
            O_Pt = pts_oneLeaf2(pts_oneLeaf2(:,4)==LeafLenPath(1+k*N),1:3);     % 骨干线第1+k*N个点作为基点，第4列是序列号
            T_Pt = pts_oneLeaf2(pts_oneLeaf2(:,4)==LeafLenPath(T_Pt_ind),1:3);  % 骨干线第一个点作为基点，第4列是序列号

            %         disp('x3')
            %         LeafLenPath(T_Pt_ind)

            [row, col] = size(pts_oneLeaf2);
            if (isempty(T_Pt) || row < 10) % 当剩余点少于10个，也直接结束
                % 下一个点找不到了，把当前的叶片点存储到直叶点数组中，然后结束循环
                pts_oneLeaf_straight(pts_oneLeaf_tr(:, 4),:) = pts_oneLeaf_tr; % 更新后的叶片坐标
                indexLeafSegs(pts_oneLeaf_tr(:, 4),k+1) = true; %  将序号保存到数组中，一列一列往右排
            ...
            if (k==0)
                O_Pt_leafbase = O_Pt; % 保存初始的叶基部第一个点
                turnAngle_leafangle = turnAngle;% 保存初始的叶角度，之后再恢复原来叶角度
            end
            % 计算叶片方向角
            ...
            % 截短叶片基部
            T_Pt_tr = pts_oneLeaf_tr(pts_oneLeaf_tr(:,4)==LeafLenPath(T_Pt_ind),1:3); % 当前截短的骨干点，第4列是序列号
            ...
            if finishFlag
                ...
                pts_oneLeaf2 = pts_oneLeaf_tr(~ind, :);    % 根据Z坐标确定 叶片上部，更新为新的pts_oneLeaf2
                pts_oneLeaf_straight(pts_oneLeaf_tr(ind, 4),:) = pts_oneLeaf_tr(ind, :); % 更新后的叶片坐标
                indexLeafSegs(pts_oneLeaf_tr(ind, 4),k+1) = true; %  将序号保存到数组中，一列一列往右排
            end
        ...
        k1 = round(k*1/3); % 叶片基部的1/3段，调节LA部分
        k2 = round(k*2/3); % 叶片中部和尖部的2/3段，调节LC的部分。
        turnAngle = leafCurvature/k2; % from 1 to k
        for ki = col:-1:1
            pts_oneLeaf2 = pts_oneLeaf_straight(logical(sum(indexLeafSegs(:,ki:col),2)), 1:3); % 取出
            if ki>k1
            ...
            pts_oneLeaf_straight(logical(sum(indexLeafSegs(:,ki:end),2)), 1:3) = pts_oneLeaf_tr; % 放回
